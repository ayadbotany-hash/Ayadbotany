...
export default function PharmaTrack() {
  ...
  const loadEmployees = useCallback(async () => {
    ...
    if (list.length === 0) {
      const admin = { name: 'المدير', username: 'admin', password: 'admin123', isAdmin: true };
      try {
        await setDoc(doc(db, EMPLOYEES_COL, 'emp_admin'), admin);
        list = [{ id: 'emp_admin', ...admin }];
      } catch (e) {}
    }
    setEmployees(list);
    return list;
  ...
  const loadVisits = useCallback(async () => {
    try {
      ...
      try {
        ...
        const list = snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
        setVisits(list);
      } catch (e2) { setVisits([]); }
  ...
  const handleAddEmployee = async () => {
    ...
    try {
      const emp = { name: newName.trim(), username: newUser.trim(), password: newPass, isAdmin: false };
